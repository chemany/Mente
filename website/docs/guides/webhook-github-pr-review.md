...
This guide walks you through connecting Mente to GitHub so it automatically fetches a pull request's diff, analyzes the code changes, and posts a comment — triggered by a webhook event with no manual prompting.
...
- Mente installed and running (`hermes gateway`)
