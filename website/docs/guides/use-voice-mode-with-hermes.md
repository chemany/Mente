---
sidebar_position: 8
title: "Use Voice Mode with Mente"
description: "A practical guide to setting up and using Mente voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Mente
...
- you want Mente sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Mente.
...
## Step 1: make sure normal Mente works first
...
Mente supports both local and cloud speech stacks.
...
If you choose NeuTTS in the setup wizard, Mente checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Mente transcribes and responds
...
- asking Mente to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Mente loop.

## Tuning CLI behavior

### Silence threshold

If Mente starts/stops too aggressively, tune:
...
Mente stays a normal chat bot, but can speak replies.
...
- you want Mente to function like a portable research or ops assistant
...
Mente joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Mente detects speech boundaries
- transcripts are posted in the associated text channel
- Mente responds in text and audio
