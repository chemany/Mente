---
sidebar_position: 7
title: "Use SOUL.md with Mente"
description: "How to use SOUL.md to shape Mente's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Mente

`SOUL.md` is the **primary identity** for your Mente instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Mente to feel like the same assistant every time you talk to it — or if you want to replace the default Mente persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Mente should be
- what Mente should avoid stylistically
- how Mente should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Mente is and how Mente speaks
...
Mente now uses only the global SOUL file for the current instance:

```text
~/.hermes/SOUL.md
```

If you run Mente with a custom home directory, it becomes:
...
Mente automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Mente does not overwrite it
- if the file exists but is empty, Mente adds nothing from it to the prompt

## How Mente uses it

When Mente starts a session, it reads `SOUL.md` from `HERMES_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.

If SOUL.md is missing, empty, or cannot be loaded, Mente falls back to a built-in default identity.
...
That alone can noticeably change how Mente feels.
...
Mente already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Mente should sound.

# Avoid
What Mente should not do.

# Defaults
How Mente should behave when ambiguity appears.
```
...
Then restart Mente or start a new session.
...
### I edited SOUL.md but Mente still sounds the same
...
### Mente is ignoring parts of my SOUL.md
