...
# 5. Symlinks the `mente` CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Mente Agent Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink mente into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up mente command..."

MENTE_BIN="$SCRIPT_DIR/venv/bin/mente"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$MENTE_BIN" "$COMMAND_LINK_DIR/mente"
echo -e "${GREEN}✓${NC} Symlinked mente → $COMMAND_LINK_DISPLAY_DIR/mente"
    ...
    if [ -n "$SHELL_CONFIG" ]; then
        ...
        if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
            if ! grep -q '\.local/bin' "$SHELL_CONFIG" 2>/dev/null; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Mente Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
...
echo "Other commands:"
echo "  mente status        # Check configuration"
if is_termux; then
    echo "  mente gateway       # Run gateway in foreground"
else
    echo "  mente gateway install # Install gateway service (messaging + cron)"
fi
echo "  mente cron list     # View scheduled jobs"
echo "  mente doctor        # Diagnose issues"
echo ""
