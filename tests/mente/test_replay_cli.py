...
def test_compare_memory_replay_reports_superseded_memory_ids():
    fixture = {
        "task": {
    ...
    result = compare_memory_replay(
        fixture,
        executor_factory=_EchoExecutor,
    )

    assert result["memory_enabled"]["selected_memory_ids"] == ["mem_new"]
    assert result["memory_enabled"]["superseded_memory_ids"] == ["mem_old"]
