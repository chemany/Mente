...
class KernelSessionRequest(BaseModel):
    """Minimal session envelope used to distinguish stateless and future session runs."""
    ...
    def _validate_session_fields(self) -> "KernelSessionRequest":
        if self.mode is KernelSessionMode.SESSION and not self.session_id:
            msg = "session_id is required when mode=session"
